# -*- coding: utf-8 -*-
"""Generator .vsdx (Visio 2013+) paketa za ER dijagram.
...
def para_sec(align=1, spline=None):
    extra = ''
    if spline is not None:      # apsolutni prorez reda u incima -> Visio ne moze da ga razvuce
        extra = C('SpLine', f(spline)) + C('SpBefore', 0) + C('SpAfter', 0)
    return ("<Section N='Paragraph'><Row IX='0'>" + C('HorzAlign', align)
            + extra + "</Row></Section>")
...
class Page:
    ...
    def label(self, x, y, w, h, text, size=7.0, style=0, align=1, color='#000000', spline=None):
        i = self._sid()
        cells = [C('PinX', f(x)), C('PinY', f(y)), C('Width', f(w)), C('Height', f(h)),
                 C('LocPinX', f(w/2)), C('LocPinY', f(h/2)), C('Angle', 0),
        ...
        s = ("<Shape ID='%d' NameU='Text.%d' Name='Text.%d' Type='Shape' LineStyle='0' "
             ...
             + ''.join(cells) + char_sec(size, style, color) + para_sec(align, spline)
             + geom_rect(w, h).replace("<Cell N='NoLine' V='0'/>", "<Cell N='NoLine' V='1'/>")
                              .replace("<Cell N='NoFill' V='0'/>", "<Cell N='NoFill' V='1'/>")
             + text_el(text) + "</Shape>")
        self.shapes.append(s); return i

    def polyline(self, pts, weight=0.01041666666666667, pattern=1, color='#000000', name='Veza'):
        ...
        cells = [C('PinX', f((bx+ex)/2.0)), C('PinY', f((by+ey)/2.0)),
                 ...
                 C('LineWeight', f(weight), 'PT'), C('LineColor', color),
                 C('LinePattern', pattern), C('LineCap', 0),
        ...
        geom = ("<Section N='Geometry' IX='0'>" + C('NoFill', 1) + C('NoLine', 0)
                + ''.join(rows) + "</Section>")
        s_ = ("<Shape ID='%d' NameU='%s.%d' Name='%s.%d' Type='Shape' LineStyle='0' "
              "FillStyle='0' TextStyle='0'>" % (i, name, i, name, i) + ''.join(cells) + geom + "</Shape>")
...
# ---------------------------------------------------------------- emit + pakovanje
STYLE0 = ("<StyleSheet ID='0' NameU='ER' Name='ER'>"
...
def pages_xml():
    return (HDR + "<Pages %s>" % NS
            + "<Page ID='0' NameU='ER' Name='ER' ViewScale='-1' "
